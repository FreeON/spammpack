...
CONTAINS
  ...
  SUBROUTINE SpAMM_Decorate_Tree2d(A)
    ...
    IF(ALLOCATED(A%Blok))THEN
       A%norm2=SUM(A%Blok**2)
       A%number_nonzeros=SIZE(A%Blok,1)*SIZE(A%Blok,2)
       RETURN
    ENDIF
  ...
  SUBROUTINE SpAMM_Decorate_Tree1d(A)
    ...
    IF(ALLOCATED(A%Blok))THEN
       A%norm2=SUM(A%Vect**2)
       A%number_nonzeros=SIZE(A%Vect,1)
       RETURN
    ENDIF
  ...
  SUBROUTINE DecorateUp_2d(A,B)
    ...
    A%nunber_nonzeros=A%nunber_nonzeros+B%nunber_nonzeros    
