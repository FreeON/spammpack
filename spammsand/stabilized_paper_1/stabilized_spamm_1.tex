...
\subsection{Staying on the Manifold}
...
\end{equation}
In the particular case where matrix $\mat{a}$ and $\mat{s}$ belong to the same manifold, $\mat{a}\, \rm{span} \,\mat{s}$, 
we interested in the braket
\begin{equation}
\left[ \tilde{\mat{a}} , \mat{s} \right]_{\tau} = \mat{\Delta}^{(\tilde{a}-\tilde{a}^\dagger) \cdot s}_{\tau} 
\end{equation}
belonging to the so(n) group.
