...
In Figures \ref{Lensing1} and \ref{Lensing2},  we demonstrate a new kind of locality that is uniquely 
exploited by $n$-body approximation of the square root iteration.
...
It is also possible to achieve lensing without regularization as shown in Figure \ref{Lensing3}.  In Figure \ref{Lensing3} however, 
lensing of the $\mat{y}_k$ channel is not as strongly contractive as in the regularized case 
(Figure \ref{Lensing2}), showing additionally the $i=k$ plane and pillae normal to $ik$.
These additional featues and delocalizations coorespond to weakness in Eq.~(\ref{bound}),  
and to the tighter thresholds required to maintain a stable iteration.  This effect is even more pronounced in the 
stable instance (not shown), where delocalizations are exagerated due to broader spectral resolutions.  

The density of pillae observed in Figure \ref{Lensing3} is associated with increasing dimensionallity and 
diminishing Cartesian seperations (nano-tube vs periodic water). For increasingly large systems, 
the decay of this density is dependent on the underlying Euclidean locality.  In Figure \ref{HilbertVsTravelOrder},
we show cull-volumes for periodic water systems ordered by the Hilbert Space Filling Curve (HSFC) \cite{} ,
and also by the Travel Order solution of the end-to-end traveling saleseman problem (TO) \cite{}  (discussed in 
Section \ref{data}).  In  Figure \ref{HilbertVsTravelOrder}, we see ....
...
\begin{figure*}[h] 
\fbox{ \includegraphics[width=7.8cm,keepaspectratio=true,
                        trim={0.cm 2.3cm 2.cm 1.cm},clip]
...
\begin{figure}[h] 
\caption{ Show percent work for H and TO orderings.}\label{HilbertVsTravelOrder}
\end{figure}
