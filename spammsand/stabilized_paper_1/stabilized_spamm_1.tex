...
remarkable algebraic structure
 ...
 128x~u.c.~nanotube, at $\sim \mat{s}_{14k \times 14k}$ and $\kappa(\mat{s})=10^6$. 
 This surface envelopes the $ijk$ (convolution) volume 
 cooresponding to the unscaled dual iteration step $\mmat{x}_{18} \leftarrow \mmat{y}_{18} \ot \mmat{z}_18 $ at $b=64$, $\tau=0.03$ and
 $\tau_y=10^{-3} \, \tau $. The left pannel looks down, from the upper bound, towards the origen and with axis normal to the view plane. 
 This axis is formed by intersection of the $ij$  and the $jk$ planes, which are followed by the occlusion surface, outside of which all
 $\tt SpAMM$ has culled all $ijk$.

 The right hand pannel looks along this axis, 
}
 \end{figure}
