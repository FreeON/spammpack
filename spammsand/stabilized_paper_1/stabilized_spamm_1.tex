...
\end{equation}
acknowledging the potential for a flexible combination of regularization and $\tt SpAMM$ approximation. The braket notation marks 
the potential for assymetries in the intermediate represenation.  This thin product representation may have advantages: 
\begin{itemize}
\item{Each solve involves a few generic and well behaved steps that may be optimized.}
\item{Each solve can be brought rapidly into compressive identity iteration.} 
\item{The {\tt SpAMM} bound is vastly strengthend,  via Eq.~(\ref{}).} 
\item{Exploits a new, strong form of locality.}
\item{The inverse factor can be applied incrementally.} 
\item{Slice update and application can be partitioned temporally.}
\end{itemize}
