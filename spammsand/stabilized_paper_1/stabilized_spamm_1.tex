...
the $\tt SpAMM$ approximation is the metric  counterpart of this locality, which can certainly be generalized
beyond Euclidean measures of distance. 

Also for the $n$-body method,  Warren and Salmon showed how to use previously accumulated histories 
(persistence, temporal locality) to rebalance and repartition the distributed task space of the octree, mapped along 
a space filling curve.  In a similar way, we showed how persistence data can be used to achieve strong parallel scaling for $\tt SpAMM$ \cite{}.  
Persistence data may also be useful in other ways that enhane mathematical approximation. 

In Figures , we show algebraic locality of the $n$-body square root iteration.
This locality develops compressively towards convergence as the contractive identity iteration develops.  
We call this compression {\em lensing},  involving deflation of the culled volume about the resolvent's plane diagonal.
This is particularly important for the product yeilding $\mat{y}_k$, because it requires a tighter threshold, $\tau_s$, 
and because iteration in the channel begins with the thicker product $\mat{s} \ots \mat{s}$.  Also, because 
spectral cancelation in the calculation of $\mat{y}^{\rm stab}_k$ is not tightly bound by Eq.~(\ref{bound}),  
significant lensing is not observed in that instance. 
...
dimensionality: non-regularized
