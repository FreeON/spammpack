...
\section{Error Flows in Accelerated Square Root Iteration}
...
\subsection{Previous errors to first order}
...
\begin{equation}
 \widetilde{\mat{z}}_{k-1} = \Delta^{\widetilde{\mat{z}}_{k-2} \cdot h_\alpha \left[ \widetilde{\mat{x}}_{k-2}\right]}_\tau 
...
+ h'_\alpha \delta \mat{y}_{k-2} \cdot \mat{z}_{k-2} 
+ \mat{y}_{k-2} \cdot h'_\alpha  \delta \mat{z}_{k-2}  \, .
\end{multline}


As above, a model for $\delta \mat{y}_{k-1}$ can be developed.  At this point however, the
distinction between different instances must be considered again.  In this study, we 
look at the ``stabilized'' version;
\begin{equation}
\widetilde{\mat{y}}^{\rm stab}_{k-1} \equiv \widetilde{\mat{z}}^{\dagger}_{k-1} \, \ot \, \mat{s}}
\end{equation}
and the cannonical ``dual'' version;
\begin{equation}
\widetilde{\mat{y}}^{\rm dual}_{k-1} \equiv  h_\alpha [ \widetilde{\mat{x}}_{k-2}] \, \ot \, \widetilde{\mat{y}}_{k-2} \, .
\end{equation}
...
\begin{equation}
\widetilde{\mat{y}}_{k-1} = \mat{\Delta}^{\widetilde{\mat{z}}^\dagger_{k-1} \cdot \, \mat{s}} \, + \,
\left( \widetilde{\mat{z}}_{k-2} \cdot h_\alpha[ \widetilde{\mat{x}}_{k-2}] \right)^\dagger \, \cdot \, \mat{s}
\end{equation}



again to first order, we take 


\begin{equation}
h^\dagger_\alpha \left[ \widetilde{\mat{x}}_{k-2} \right]   =  
h_\alpha \left[ \mat{x}_{k-2} \right] +  h'_\alpha  \delta \mat{x}^\dagger_{k-2} 
\end{equation}


\begin{multline}
\delta \mat{y}_{k-1}= \mat{\Delta}^{\widetilde{\mat{z}}^\dagger_{k-1} \cdot \, \mat{s}}_\tau +
h'_\alpha \delta \mat{x}^\dagger_{k-2} \cdot \mat{y}_{k-2}  \\+ h_\alpha [ \widetilde{\mat{x}}_{k-2}] \cdot \delta \mat{z}_{k-2} \cdot \mat{s}
\end{multline}

\begin{multline}
\delta \mat{y}_{k-1}= \mat{\Delta}^{\widetilde{\mat{z}}^\dagger_{k-1} \cdot \, \mat{s}}_\tau \\ +
h'_\alpha \left(
\mat{z}^\dagger_{k-2} \cdot \delta \mat{y}^\dagger_{k-2}+
\delta  \mat{z}^\dagger_{k-2} \cdot \mat{y}^\dagger_{k-2}
 \right)  \cdot \mat{y}_{k-2}  \\+ h_\alpha [ \widetilde{\mat{x}}_{k-2}] \cdot \delta \mat{z}_{k-2} \cdot \mat{s}
\end{multline}

\subsubsection{dual}

\begin{equation}
\widetilde{\mat{y}}_{k-1} = \mat{\Delta}^{  h_\alpha[ \widetilde{\mat{x}}_{k-2}] \cdot \widetilde{\mat{y}}_{k-2} }
 \, + \,
