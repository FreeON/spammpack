...
Even for the most approximate but stable (MAYSS) approximations, our nanotube calculations are dense, even for very large, $128 x$ U.C. systems. 
For ill-conditioned square root iterations, the corresponding broad spectral resolutions, involving products of large and small numbers, 
coorespond to delocalized products that are not tightly bound by the $\tt SpAMM$ approximation.   

One way to reduced ill-conditioning, tightening Eq.~(\ref{bound}), is through Tikhonov regularization \cite{}.
...
\subsection{Most Approximate by One Order}
