...
Sparse schemes based on matrix truncation control only {\em absolute, addititve errors} in the matrix product,   
...
before sparsity can be achieved.  However, extended degeneracies  are often amenable to hierarchical,  compressive 
...
naive ${\mathcal O}(n^3)$  of conventional dense ${\tt GEMM}$ kernels, with interesting fast stragies for achieving fast accumulation
of product space intermediates \cite{Demmel et. al.}. 
However, many pratical cases are somewhere in-between sparse and meaningfully degenerate regimes.   This is the case in electronic structure 
towards the Mott transition \cite{}, and also in the case of local support towards completeness \cite{Others, Hutter, Gigi}. 
...
\end{equation}
and orderings that are locality preserving \cite{}, providing matrices with approximate, block-by-magnitude structuring as described in Refs. \cite{}.
...
and supporting both sparse \cite{wise} and compressive \cite{} approaches to linear algebra.   With matrix quadtrees structured block-by-magnitude, 
the Sparse Approximate Matrix Multiplication ($\tt SpAMM$) kernel $\ot$ achives a reduced ${\cal O}(n)$ cost through recursive Cauchy-Schwarz 
...
\end{equation}
\end{widetext}
with $\lVert \cdot \rVert \equiv \lVert \cdot \rVert_F$ because computability \cite{kahan}.  In addition to the potential of an 
...
Perhaps most importantly though, the occlussion error 
\begin{equation}
...
\end{equation}
with the potential for {\em relative} error control during iteration.   Generally, the opperator $\ot$ leads to 
a non-associative algebra with Lie bracket
\begin{equation}
...
Our challenge is to master the error flows of these occlusion fields under iteration, 
