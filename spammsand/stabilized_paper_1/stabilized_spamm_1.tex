...
correlations between support functions and the representation independent form [Lowdin], {\em eg.} of the generalized eigenproblem.    
These matrix functions are related through Higham's identity, connecting the matrix sign function with the inverse square root:
\begin{equation}
\rm{sign} \left( \begin{bmatrix} 0 & \mat{s}      \\ \mat{I}       & 0\end{bmatrix} \right)  =
                 \begin{bmatrix} 0 & \mat{s}^{1/2} \\ \mat{s}^{-1/2} & 0\end{bmatrix}  .
\end{equation}

A well conditioned matrix $\mat{s}$ corresponds to rapid decay, and may be approximated with the sparse matrix 
$\bar{\mat{s}} = \mat{s}+ \mat{\epsilon}^{\mat{s}}_\tau$, where $\mat{\epsilon}^{\mat{s}}_\tau$ is the error introduced according to some 
criteria $\tau$.   This criteria might be a droptolerence, 
...
or some other {\em ad hoc} approach to truncation.  Then, conventional kernels may be employed, such as the Sparse Matrix Multiply 
($\tt{SpMM}$), yeiding fast solutions for  multiplication rich iterations. 
