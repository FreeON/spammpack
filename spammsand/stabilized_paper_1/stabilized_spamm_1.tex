...
Cartesian distance between support functions, or it may involve a statistical distance, {\em e.g.}~between strings.  In electronic structure, 
correlations manifest in decay properties of the matrix 
sign function, as projector of the effective Hamiltonian (See Fig. \ref{figure1}.  
More broadly, matrix decay properties may coorespond to learned correlations in a 
...
are often solved with congruential transformations based on the matrix inverse square root; these  transformations determine
correlations between the local support and the representation independent form [Lowdin], {\em eg.} of the generalized eigenproblem.    
Higham's identity relates the matrix sign function with the matrix inverse square root:
\begin{equation}
\rm{sign} \left( \begin{bmatrix} 0 & \mat{s}      \\ \mat{I}       & 0\end{bmatrix} \right)  =
                 \begin{bmatrix} 0 & \mat{s}^{1/2} \\ \mat{s}^{-1/2} & 0\end{bmatrix}  .
\end{equation}
A complete overivew of matrix function theory and computation is given in Higham's excellent reference \cite{}. 

A well conditioned matrix $\mat{s}$ may correspond to matrix sign and inverse square root functions with rapid exponential decay, 
...
criteria $\tau$.   This criteria might be a drop-tolerence, 
...
Then, conventional computational kernels may be employed, such as the Sparse Matrix Multiply 
($\tt{SpMM}$) \cite{Gustavson, Toledo, Buluc}, yeiding fast solutions for multiplication rich iterations and a modulated fill in. 
...
other disciplines.  A comprehensive overview of these methods in the numerical linear algebra is given by Benzi \cite{}. 
See also Bowler for a complete development of these methods in electronic structure \cite{, ARCHIVE_PRL}, and a current summary of
high performance results \cite{}.


\begin{figure}[h]
 \includegraphics[width=3.5in]{decay_picture.png}
  \caption{Examples from electronic structure of decay for the spectral projector (gap shifted sign function) with respect to local (atomic) support.  Shown is decay   
           for systems with correlations that are short (insulating water), medium (semi-conducting 4,3 nanotube), and long (metalic 3,3 nanotube) ranged,  from exponential (insulating) to algebraic (metallic). }
\end{figure}
