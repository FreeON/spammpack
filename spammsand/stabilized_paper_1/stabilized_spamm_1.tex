...
vastly strengthening the bound  Eq.~(\ref{bound}) and exploiting locality properties about
the fixed point, where NS is strongly contractive about the diagonal.  Then, 
it may be possible to iteratively build the full factor with likewise ``thin'', generic 
increments ($\Delta \tau \sim .1$ and $\Delta \mu \sim .1$). 

The first of these increments is shown in 

Cooresponding to the (3,3) $\kappa(\mat{s})=10^{10}$ nanotube series with $N={\rm  LIST MATRIX DIMS}$ 
shown in Figs.~\ref{flow_noscale_dual}-\ref{flow_scaled_stab}, 


Compression of culled $\tt SpAMM$ volumes is shown for a first thin slice 
is shown in Fig.~\ref{regularized_stab} for the stable instance, and in 
Fig.~\ref{regularized_dual} for the dual instance, 
...
TALK ABOUT COMPRESSION RIGHT HERE.  COMPRESION ON X->I & STONGLY CONTRACTIVE  -> ACCELERATION FOR THIN SLICE

Acceleration is marked by rapidly decreasing culled volumes with respect to problem size, {\em e.g.}  ${\rm vol}_{\mat{a} \ot \mat{b}}/N^3$, 
Starting with the ill-conditioned nanotube matrices looked at in the previous section, Figs.~\ref{}-\ref{}, 
we show thin slice volumes for the formation of $\mat{y}_k$ and $\mat{z}_k$ in Fig.~\ref{regularized_stab} for the stable instance, 
and in Fig.~\ref{regularized_dual} for the dual instance.  These instances behave very differently; in the case of the stable
iteration, 

WHILE THE EIGEN STRUCTURE IS MODIFIED, THE NORMS IN EG S ARE NOT.  THUS, FOR LARGER SYSTEMS THE STAB METHOD DOES WORSE AND WORSE,
B/C IT DOES NOT AVAIL OF THE SPAMM BOUND!!


 in this case.  In the first instance, the $\mat y$ channel is unimproved and
...
In the dual case then,  $\tau_0$ can be brought to extreme permisve values, $\tau_0 = .1$, for {\em e.g.} $\mu_0 = .1$.
Then, from a first generic slice $\mat{s}^{-1/2}_{\tau_0, \mu_0}$,  the next level shift, $\mu_1= .01$, can to obtain a next effective preconditioner, 
$\mat{s}^{-1/2}_{\tau_0 \mu_1}$, based on the residual $\left(\mat{s}^{-1/2}_{\tau_0\mu_0} \right)^\dagger \, \oto \, \left(\mat{s}+\mu_1 \mat{I} \right) \, 
\oto \, \mat{s}^{-1/2}_{\tau_0\mu_0} $.  Generally, it may be possible to find the full factor 
as the nested product of generic thin slices ($\tt SpAMM$ sandwitch):
\begin{eqnarray}
