...
matrix compression \cite{} in a step that preceeds multiplication.   However, fast methods for matrix compression are 
generally compatible with the matrix quadtree, as are additional fast (generalized) solvers.  Thus, generality 
and interoperability may enable powerful solver ecosystems through deeply layered approximations to in place data. 
Language support for generic recursion may also support complex solver ecosystems with simple (skeletonized) 
frameworks, lowering barriers to entry, enhancing concurence in the Erlang sense, and following a sustainable 
commodity trend \cite{softwaresustainanbilty}.
