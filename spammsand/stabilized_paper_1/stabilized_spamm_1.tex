...
Show figures for TO vs SFC\\[.5in]
...
It is also possible to achieve similar (sparse) results without regularization at greater 
effective length scales, as shown in Figure \ref{}.  In addition to algebraic and Euclidean locality, other 
measures are of interest, including information theoretical measures, generalized space filling curve 
heuristics, as well as graph theoretical approaches that localize matrix elements about the diagonal \cite{}.  
In figure \ref{}, we show the unregularized for the $\kappa(\mat{s})=10^{10}$ structural matrix ${\tt bb}$. 
...
by incremental application to a target vector or matrix}
\item{Potential for incremental preconditionin on multiple time scales (most approximate cooresponding to longest time scale)}
\item{solution to problem developed from on genericity and concurence of the Erlang kind. Ecosystems impact.
...
\item{problems in electronic structure,generalized polar decomposition} and  ill-conditioning in radial basis function problems}
\end{itemize}
