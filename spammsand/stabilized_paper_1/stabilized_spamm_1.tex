...
\end{equation}
An alterative approach is to find a reduced rank approximation closed under the opperations of interest. 
...
Increasing data concentration allows to avoid white space in querries and 
computation, which is compressive for algorithms that can exploit these properties.
One common example of such data locality cooresponds to an underlying order
...
We again ammend the $\tt SpAMM$ occlusion-cull,  with the recursion
\begin{widetext}
\begin{equation}
\mat{a}^{i} \ot \mat{b}^{i} =
\left\{
...
while in the $\tt SpAMM$ approach to fast multixplication, the numerically least significant 
tensor intermediates in the n\"{a}ive product space are ommited recursively, with error bounded by Eq.~(\ref{bound}).  
These methods for fast matrix multiplication are {\bf \em stable} \cite{DDH07}.
...
generalization reflects the {\em genericity} of recursive data access 
\cite{genericityindata,Geerts2002,Samet:2006:DBDS,Gottschling2009},
enabling new math supported by range querries, metric querries, higher dimensional querries and so on, but with common frameworks, 
structures and runtimes.  So far, we have prototyped $n$-body solvers for the  mainstay problems in modern electronic structure theory, 
...
skelitinizations \cite{Sarje2010a,Gonzalez-Velez:2010:SAS:1890754.1890757,CPE:CPE3087,rodrigues2014triolet,March} 
may enable expoitation of increasingly cheap (decentralized) concurence by scientific $n$-body solvers, 
and also lower bariers to entry, enhance reliability, sustainability \& {\em etc.}
...
Recently for example, Driscoll {\em et. al} showed perfect strong scaling and communication optimality 
...
strong parallel scaling for $\tt SpAMM$ with commonly available runtimes \cite{Bock13}.  
