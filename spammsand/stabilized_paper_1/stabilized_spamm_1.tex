...
The focus of our research is experimental,  so far involving issue driven implementations of $\tt SpAMM$, 
including a Haskell version (formal functional programming) \cite{}, 
a fine grained ($\tt 4\times 4$) {\tt s.p.}~assembly coded version (scalar performance) \cite{} and  
a task parallel version in {\tt C++}, {\tt OpenMP 3.0} and {\tt Charm++} (strong scaling) \cite{}.  
In the current contribution, informal functional progamming in {\tt Fortran08} \cite{} was used, 
with the goal of demonstrating generic simplicity and mathematical development.
...
\end{equation}
and the stability parameter to zero,-box 
\begin{equation}
...
{\tt TubeGen} \cite{tubegen}
STO-2G \cite{Schuchardt2007}

with diffuse $sp$-shell, with $\zeta_{10^{10}}={\tt .06918}$, $\zeta_{10^{11}}={\tt .05934}$
double exponential .  We are not advocating such ill-conditioned functions as an efficient basis; but are using artificially difficult problems to illustrate assymptotically bad .. 

the triple $\zeta$ 6-311G** basis set \cite{Schuchardt2007} 

the {\tt gromacs} utility {\tt gmx solvate -box} \cite{gmx30}
to generate cubic water box coordinates increasing by 100, and cooresponding to standard temperature and preasure. 
