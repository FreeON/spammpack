...
\caption{
Culled volumes for the XXx U.C. (3,3) $\kappa(\mat{s})=10^{10}$ nanotube overlap matrix examined in the previous section,  
in the stable instance and with regularization ($\mu_0=.1$): 
$\rm v_{\widetilde{\mat{z}_k}} = \left( {\rm vol}_{ \widetilde{\mat{z}}_{k-1}\ot h[\widetilde{\mat{x}}_{k-1}] } \right) \times 100\% /N^3$, 
...
\caption{
Culled volumes for the XXx U.C. (3,3) $\kappa(\mat{s})=10^{10}$ nanotube overlap matrix examined in the previous section,  
in the dual instance and with regularization ($\mu_0=.1$): 
${\rm v}_{\widetilde{\mat{y}}_k}= \left( {\rm vol}_{  h[\widetilde{\mat{x}}_{k-1}] \ots \widetilde{\mat{y}}_k }  \right) \times 100\% /N^3$, 
...
$\widetilde{\mat{y}}^{\rm stab}_k \rightarrow \mat{s}^{1/2{}_{\tau_0 \mu_0}}\equiv \mat{s}^{-1/2}_{\tau_0 \mu_0} \oto \mat{s}_{\mu_0}$ is 
a product not helped by regularization.  In the dual case the situation is quite different with
$\widetilde{\mat{y}}^{\rm dual}_k \rightarrow  \mat{I}_{\tau_0 \mu_0} \oto \mat{s}^{1/2}_{\tau_0 \mu_0}$, which is reflective about the cube
...
$\mat{s}^{-1/2}_{\tau_0 \mu_1}$, based on the residual $\left(\mat{s}^{-1/2}_{\tau_0\mu_0} \right)^\dagger \, \oto \, \left(\mat{s}+\mu_1 \mat{I} \right) \, 
\oto \, \mat{s}^{-1/2}_{\tau_0\mu_0} $, again with an effective increment $\mu_1 < .1 \mu_0$.  Generally then, it may be possible to find the full factor 
...
\end{eqnarray}with $1 > \tau_0 > \tau_1 > \dots > \tau_n $. 
Certainly, there are many ways to combine $\tt SpAMM$ resolutions with Tikhonov regularization. 
...
({\bf 5}) Each generic solve may achieve additional  computational and mathematical acceleration for the next solve through 
persistence (Section \ref{}).
