...
The $\tt SpAMM$ querry hierarchically resolves complex algebraic structures in the $ijk$ product cubes of square root iteration,  
...
these algebraic structures contract further as multiplication by near-identity is reached, with
...
cannonical ``dual'' instance, Eq.~(\ref{}), and for the ``stable'' instance, Eq.~(\ref{}).
Consistent with HMMT's results \cite{}, we find terms in the $\mat{z}_k$ channel with sensitivity to the full inverse,
$\mat{s}^{-1}$, and proscribe a tighter threshold for that case, $\tau_s \ll  \, \times \, \tau$.  Later, we find that 
constraint is strongly mitigated by lensing.  In this Section, we also looked at bifurcations of scaled and unscaled iterations
for ill-conditioned systems, towards a most-approximate-yet-still-stable (MAYSS) preconditioner. 
...
We looked at the MAYEBOO approximation for both the stable and dual instances, and found that even with the most
permisive regularization, spectral resolution in the stable instance is too broad to achieve strong lensing.  
% $\mat{I}(\mat{s}) =  
