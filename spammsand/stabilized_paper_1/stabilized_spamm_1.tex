...
SOMETHING ABOUT METRIC QUERRIES HERE...
...
and to the tighter thresholds required to maintain a stable iteration in the MAYSS approximation.  This effect is even more pronounced in the 
stable instance (not shown), where delocalizations are exagerated due to spectral resolutions that are broader.  
...
system size $n$.  
Also, it may be possible to sample pillae effectively with non-determistic methods, involving {\em e.g.}~probabilities accumulated through persistence.

In Figures \ref{Lensing1} and \ref{Lensing2},  we show a new kind of locality that is uniquely exploited by $n$-body approximation of the square root iteration.
...
%CASE OF WELL CONDITIONED PROBLEMS, MAYEBOO
%SAY SOMETHING ABOUT FULL COMPRESSION TO THE PLANE DIAGONAL 

%THIS IS THE ADDITIONAL ACCELERATION POSSIBLE BY MORE APPROXIMATE SPAMM AND SLAMMING THE DOOR SHUT ON DERIVATIVES;
%TIGHT CONVERGENCE OF THE DIRECTIONAL DERIVATIVES, Eqs ~\ref{} and \ref{}. 

This is important for the product yeilding $\mat{y}_k$, because iteration in the channel begins with the 
thicker product $\mat{s} \ots \mat{s}$ and involves the tighter threshold $\tau_s \sim 0.01 \times \tau$. 
...
In this contribution, we developed an $n$-body solver for square root iteration that expoits  
metric locality, cooresponding to decay with Cartesian or non-Euclidian seperation, 
and also algebraic locality, developing with identity iteration.  The generic $n$-body
approach carries out a two-sided metric querry of with the modified $\tt SpAMM$ criteria, ,
with cull-volumes that are 

directional derivatives of the square root iteration, Eqs~\ref{}-\ref{}, 

strongly compressed along $ijk$ face and cube diagonals in  


based on a compresive,
lensing approach to fast matrix multiplication.  copy in place, pivot on diagonal, row col cannot resolve.
note potential to employ reductions in adjoint volume, non-deterministic, and locally random approaches 
that can be layered in parallel onto in place data. 



new bound for N-Body approach to fast matrix multiplication: methods that truncate the data do not bound the outcome

stability analyses and bifurcations of ill-conditioned spamm away from basin of stability 

show how to go past these bifurcations using regularization.
...
shows two new kinds of locality: strong Euclidean locality based on 
...
product representation of generic MAYEBOO slices with 
...
Sandwich approach potential for defered, implicit delocalization of the inverse factor, that can be avoided
...
all about shuting the door on directional derivatives.
