    ...
    struct spamm_mm_t *linear_tree_memory, struct spamm_node_t *node)
{
  ...
  else
  {
    ...
    if (node->block_dense != NULL)
    {
       ...
       * padding.
       */
      linear_block = (struct spamm_linear_quadtree_t*) spamm_mm_allocate(sizeof(struct spamm_linear_quadtree_t)+SPAMM_N_BLOCK*SPAMM_N_BLOCK*sizeof(floating_point_t)+8, linear_tree_memory);
      ...
      for (i = 0; i < SPAMM_N_BLOCK; ++i) {
        ...
        {
          linear_block->block_dense[spamm_dense_index(i, j, SPAMM_N_BLOCK, SPAMM_N_BLOCK)] = node->block_dense[spamm_dense_index(i, j, SPAMM_N_BLOCK, SPAMM_N_BLOCK)];
        }
      }
    }
  }
