    ...
    if (node->child[0] == NULL)
    {
      ...
      for (l = 0; l < 2; ++l) {
        for (k = 0; k < 2; ++k)
        {
          ...
          node->child[spamm_dense_index(l, k, 2)]->N_lower = node->N_lower+l*(node->N_upper-node->N_lower)/2;
          node->child[spamm_dense_index(l, k, 2)]->N_upper = node->N_lower+(l+1)*(node->N_upper-node->N_lower)/2;
