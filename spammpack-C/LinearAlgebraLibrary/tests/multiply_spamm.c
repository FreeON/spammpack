...
main ()
{
  ...
  int max_block = 4;
  int M_block[4] = { 1, 2, 10, 100 };
  int N_block[4] = { 1, 2, 10, 100 };
      ...
      for (i_block = 0; i_block < max_block; ++i_block) {
        for (i_child = 0; i_child < max_child; ++i_child)
        {
          ...
          for (i = 0; i < M[i_size]; ++i) {
            for (j = 0; j < N[i_size]; ++j)
            {
              if (spamm_get(i, j, &C) != spamm_get(i, j, &C_test))
              {
                printf("[multiply_spamm] mismatch: (C_test[%i][%i] = %e) != (C[%i][%i] = %e)\n", i, j, spamm_get(i, j, &C_test), i, j, spamm_get(i, j, &C));
                exit(1);
              }
            }
          }
