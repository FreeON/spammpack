...
main ()
{
  ...
  int max_fill_A = 4;
  double fill_A[4] = { 0.01, 0.2, 0.5, 1.0 };

  int max_fill_B = 4;
  double fill_B[4] = { 0.01, 0.2, 0.5, 1.0 };
      ...
      for (i_size = 0; i_size < max_size; ++i_size) {
        for (i_block = 0; i_block < max_block; ++i_block) {
          for (i_child = 0; i_child < max_child; ++i_child)
          {
            ...
            //LOG("%ix%i matrix, A = %.1f%% full, B = %.1f%% full, padded %ix%i, i_block dimensions %ix%i, i_child dimensions %ix%i\n",
            ...
            //LOG("A: nodes = %i, blocks = %i, tree = %i bytes, blocks = %i bytes (%1.1f%%)\n", stats.number_nodes, stats.number_dense_blocks,
            //    stats.memory_tree, stats.memory_dense_blocks, (stats.memory_tree+stats.memory_dense_blocks)/(double) (M[i_size]*N[i_size]*sizeof(double))*100);
            //spamm_tree_stats(&stats, &B);
            //LOG("B: nodes = %i, blocks = %i, tree = %i bytes, blocks = %i bytes (%1.1f%%)\n", stats.number_nodes, stats.number_dense_blocks,
            //    stats.memory_tree, stats.memory_dense_blocks, (stats.memory_tree+stats.memory_dense_blocks)/(double) (M[i_size]*N[i_size]*sizeof(double))*100);
            //fflush(stdout);
