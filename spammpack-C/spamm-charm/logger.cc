    ...
    const char *const format, ...)
{
  ...
  if(strlen(tag) > 0)
  {
    snprintf(new_format, format_length, "[%s:%d (%s) %1.2fs PE:%d %s] %s", filename,
        linenumber, function_name, walltime, CkMyPe(), tag, format);
  }

  else
  {
    snprintf(new_format, format_length, "[%s:%d (%s) %1.2fs PE:%d] %s", filename,
        linenumber, function_name, walltime, CkMyPe(), format);
  }
